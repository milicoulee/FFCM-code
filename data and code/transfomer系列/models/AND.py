...
class BiLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size, dropout=0.2):
        ...
        self.fc = nn.Linear(hidden_size * 2, output_size)  # 双向 LSTM 的输出维度是 hidden_size * 2
        # self.lstm = nn.LSTM(input_size, hidden_size, num_layers,
        #             batch_first=True, bidirectional=False, dropout=dropout)
        # self.fc = nn.Linear(hidden_size, output_size)  # 输出维度调整为 hidden_size
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.bilstm = BiLSTM(input_size=configs.d_model, hidden_size=configs.d_model // 2, num_layers=2,
                            output_size=configs.pred_len, dropout=configs.dropout)
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            ...
            self.projection2 = nn.Linear(1, 4, bias=True)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
      ...
      ###如果需要做消融实验吧这个209-213注释就行
        ...
        # enc_out, attns = self.encoder(enc_out, attn_mask=None) #torch.Size([64, 9, 64]) B 输入长度 模型维度
        ...
        # Apply projection and BiLSTM
        # proj_out = self.projection(enc_out).permute(0, 2, 1)  # [B, L, D] -> [B, D, L]
        
        lstm_out = self.bilstm(enc_out)  # Pass through BiLSTM
        lstm_out = lstm_out.unsqueeze(1)  # Ensure it's in the correct shape for final projection
        lstm_out = lstm_out.permute(0, 2, 1)
        lstm_out=self.projection2(lstm_out)
        # Final output should be dec_out
        dec_out = lstm_out  # Use lstm_out as dec_out
