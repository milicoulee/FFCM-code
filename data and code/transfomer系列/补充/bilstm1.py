...
# selected_features =  ['风速', '预测功率', '实际风速', 'Target']
# # selected_features = [1, 2, 3, 'Target']
...
#         y.append(data[i + input_window:i + input_window + output_window, :])  # 输出的维度为 6 × 1
...
class BiLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size, dropout=0.1):
        ...
        self.fc = nn.Linear(hidden_size * 2, 4)  # 4就是特征数
        self.conv1 = nn.Conv1d(in_channels=input_size, out_channels=64, kernel_size=3, padding=1)
        self.conv2 = nn.Conv1d(in_channels=64, out_channels=4, kernel_size=3, padding=1)
        # self.lstm = nn.LSTM(input_size, hidden_size, num_layers,
        #                     batch_first=True, bidirectional=False, dropout=dropout)
        # self.fc = nn.Linear(hidden_size, 4)  # 4就是特征数

        self.dropout = nn.Dropout(p=dropout)  # 设置 dropout 概率
    ...
    def forward(self, x):
        ...
        out, _ = self.lstm(x)
        # print(out.shape)
        # 取最后一个时间步的输出（历史滑动窗口的最终特征）
        out = out[:, -1:, :]  # 形状: (batch_size, hidden_size * 2)  输出长度
        # print(out.shape)
...
for epoch in range(num_epochs):
    ...
    model_filename = os.path.join('./保存最后10个/LSTM/', f"LSTM_epoch_{epoch + 1}.pth")
...
# 7. 测试模型
# save_path = "./save/BILSTM/6.pth" 
# save_path = "./保存最后10个/LSTM/LSTM_epoch_30.pth" 
save_path = f"./保存最后10个/LSTM/LSTM_epoch_{num_epochs}.pth"
...
#     'Predict': pred_uninverse[-1000:, -6].flatten(),
#     'Real': true_uninverse[-1000:, -6].flatten()
# })
...
for epoch in range(a, b):  # 从epoch 21到30
    save_path = f"./保存最后10个/LSTM/LSTM_epoch_{epoch}.pth"  # 保存的模型路径
    loaded_model =BiLSTM(input_size, hidden_size, num_layers, output_size).to(device)  # 使用与训练时相同的模型架构
