...
from models import iTransformer,iReformer,iFlashformer,iFlowformer,iInformer,Informer,AND
...
window = 16  # 模型输入序列长度 这里只能是 12345 的这种次方倍数 这个改的话 model层的209行也得改
length_size = 3  # 预测结果的序列长度
batch_size = 64
...
num_epochs = 50   # 训练迭代次数 300论以后趋于稳定 0.001+500最佳 这个别改 loss维持再0.0010徘徊
...
for repeat in range(num_repeats):
    ...
    # net = AND.Model(config).to(device)
    ...
    # save_path = "./保存最后10个/model_epoch_21.pth" 
    r_squared_values = [] 
    mse_values = []
    mae_values =[]
    a,b = 1,101
...
import pandas as pd
r_squared_series = pd.Series(r_squared_values, index=range(a, b))
mse_series = pd.Series(mse_values, index=range(a, b))
...
# plt.plot(mse_series.index, mse_series.values, marker='s', color='g', linestyle='-', label="MSE", markersize=6)
